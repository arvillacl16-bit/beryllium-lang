...
struct Token {
  ...
  std::string to_string() const {
    ...
    case SEMI:
      return "SEMI";
    case INT_LIT:
      return "INT_LIT(" + metadata + ")";
    case FLOAT_LIT:
      return "FLOAT_LIT(" + metadata + ")";
    case STR_LIT:
      return "STR_LIT(" + metadata + ")";
    case CHAR_LIT:
      return "CHAR_LIT(" + metadata + ")";
    case VAR:
      return "VAR";
    case IDENT:
      return "IDENT(" + metadata + ")";
    case IF:
      return "IF";
    case WHILE:
      return "WHILE";
...
public:
  ...
  explicit TokenStream(std::vector<Token> &&toks)
      : tokens(std::move(toks)), cursor(0) {}
  explicit TokenStream(const std::vector<Token> &toks)
      : tokens(toks), cursor(0) {}
  ...
  public:
    explicit Iterator(const TokenStream &stream)
        : data(&stream.tokens), cursor(stream.cursor), offset(0) {}
