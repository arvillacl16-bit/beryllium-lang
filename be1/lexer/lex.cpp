...
const Token &TokenStream::advance() {
  if (!has_next())
    beryl::throw_lex_error("End of token stream reached", -1, -1);
  ...
  while (cursor < buf.length()) {
    ...
    if (peek() == '"') {
      ...
      while (peek() != '"' && cursor < buf.length()) {
        if (peek() == '\\' && peek(1) == 'n') {
          ...
          beryl::throw_lex_error("Unknown escape sequence in string literal",
                                 line, col);
        }
      }
      if (cursor >= buf.length()) {
        beryl::throw_lex_error("Unterminated string literal", tok.line,
                               tok.col);
      }
      ++cursor;
      ++col;
    ...
    if (peek() == '\'') {
      ...
      if (peek() == '\\') {
        if (peek(1) == 'n') {
          ...
          beryl::throw_lex_error("Unknown escape sequence in character literal",
                                 line, col);
        }
      } else {
        tok.metadata = peek();
      ...
      if (peek() != '\'') {
        beryl::throw_lex_error("Unterminated character literal", tok.line,
                               tok.col);
      }
      ++cursor;
      ++col;
    ...
    if ((peek() == '-' && std::isdigit(peek(1))) || std::isdigit(peek())) {
      ...
      while (std::isdigit(peek()) || peek() == '.') {
        if (peek() == '.') {
          if (is_float) {
            beryl::throw_lex_error("Multiple decimal points in number literal",
                                   line, col);
          }
          is_float = true;
        }
    ...
    if (cursor + 1 < buf.length()) {
      ...
      if (auto it = double_symbols.find(potential_double);
          it != double_symbols.cend()) {
        Token tok;
        tok.type = it->second;
        tok.line = line;
        tok.col = col;
    ...
    if (!matched_symbol) {
      if (auto it = symbols.find(peek()); it != symbols.cend()) {
        ...
        beryl::throw_lex_error(std::string("Unknown symbol: ") + peek(), line,
                               col);
      }
    }
